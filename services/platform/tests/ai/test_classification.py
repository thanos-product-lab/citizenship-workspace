...
def test_a_supported_document_is_classified_and_recorded(ai_settings: Settings) -> None:
    provider = FakeProvider(responses=[succeeded(_output(ClassifiedCategory.TRAVEL_SUPPORT))])
    outcome = _classify(provider, ai_settings)
...
def test_declining_to_choose_is_recorded_as_abstention_not_failure(
    ai_settings: Settings, category: ClassifiedCategory
) -> None:
...
def test_a_low_confidence_answer_is_still_recorded_as_an_answer(ai_settings: Settings) -> None:
...
def test_a_spent_budget_is_a_state_not_an_exception(ai_settings: Settings) -> None:
...
def test_an_exhausted_task_deadline_is_a_state(ai_settings: Settings) -> None:
    outcome = _classify(FakeProvider(responses=[]), ai_settings, budget_seconds=0.5)
...
def test_a_provider_failure_invents_no_category(ai_settings: Settings) -> None:
...
def test_a_refusal_invents_no_category(ai_settings: Settings) -> None:
    """AI_EVALUATION_PLAN §8.14: a refusal is a recoverable state and never a
    fabricated fallback."""
    outcome = _classify(FakeProvider(responses=[failed(ModelRunStatus.REFUSED)]), ai_settings)
    assert outcome.run.status == ExtractionRunStatus.FAILED.value
    assert outcome.category is None
...
def test_the_input_is_capped(ai_settings: Settings) -> None:
    ...
    answer."""
    provider = FakeProvider(responses=[succeeded(_output(ClassifiedCategory.TRAVEL_SUPPORT))])
    _classify(provider, ai_settings, text="A" * 100_000)
...
def test_the_run_records_a_hash_of_exactly_what_was_sent(ai_settings: Settings) -> None:
    provider = FakeProvider(responses=[succeeded(_output(ClassifiedCategory.TRAVEL_SUPPORT))])
    outcome = _classify(provider, ai_settings, text="B" * 50_000)
...
def test_the_capability_is_recorded_on_the_run(ai_settings: Settings) -> None:
